...
class ShopService(val location: Location) {
    ...
    private suspend fun ramenBackgroundTask(ramenUrl: Distance?): String {
        ...
        val response = withContext(Dispatchers.IO) {
            var httpResult = ""
            try {
                val urlObj = URL(ramenUrl?.requestUrl(location))
    ...
    enum class Distance(private val range: Int) {
        ...
        fun requestUrl(location: Location): String {
            return "${mainUrl}${apiKey}&lat=${location.latitude}&lng=${location.longitude}&range=${range}&format=json"
        }
    }
